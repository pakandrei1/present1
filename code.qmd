...
| __`median_income`__ | Median income for households within a block of houses (measured in tens of thousands of US Dollars) |
| __`median_house_value`__ | Median house value for households within a block (measured in US Dollars) |
...
s1 = pd.Series(dataset['median_house_value'],
               name='Before')
...
$$A = \text{income} > 100 , 000$$
$$B = \text{median house value} > 300 , 000$$
```{python}
#| scrolled: false
# P(B|A) = P(A & B) / P(A),
...
```{python}
res = stats.linregress(dataset['median_income'], dataset['median_house_value'])
print(f"""\
Slope: {res.slope:.10f}
...
## Line regression and prediction (scatterplot & line)
```{python}
#| output-location: slide
y_real = pd.Series(dataset['median_house_value'], name="Real value")
y_pred = pd.Series(res.slope*dataset['median_income'] + res.intercept, name="Predicted from income")
...
r, p = stats.pearsonr(dataset['median_income'], dataset['median_house_value'])
